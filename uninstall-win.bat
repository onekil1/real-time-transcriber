@echo off
rem ==========================================================
rem  Транскрибатор - ПОЛНОЕ удаление установки на Windows.
rem  Стирает всё в папке проекта (модели, БД, venv, .git, код)
rem  и оставляет только этот файл - чтобы можно было сразу
rem  поставить заново через git clone + install.bat.
rem  Модель Whisper при следующем install подтянется автоматом.
